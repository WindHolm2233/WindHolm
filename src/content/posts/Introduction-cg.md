...
pinned: false
description: 一篇面向入门者的计算机图形学学习概论，覆盖流水线、光栅化、变换和渲染基础。
...
计算机图形学关注的是如何用计算机表示、生成、处理并显示图形。它和数字图像处理不同：前者更强调从几何模型出发“生成图像”，后者更关注对已有像素做编辑和增强。

这篇文章把入门内容拆成四个部分：流水线、光栅化、几何变换，以及三维渲染的基础思路。先建立整体框架，再去看细节会更容易进入状态。

## 一、图形系统与可编程流水线

### 核心概述
图形学的核心任务，是把抽象的几何数据转换成屏幕上的像素结果。这个过程通常会经历顶点处理、图元装配、光栅化、片元处理和逐像素操作，最后写入帧缓冲区。

### 技术深度
这一领域建立在 **图形流水线（Graphics Pipeline）** 之上。它是一个高度并行的阶段式流程，数据通常从顶点开始，依次经过：
...
- **硬件进化**：早期图形卡主要依赖固定功能管线，很多工作都由 CPU 完成。现代 GPU 则拥有大量并行核心，可以同时处理大量顶点和片元。
- **编程范式**：可编程流水线和 GLSL 已经成为主流。开发者可以自定义：
  - 顶点着色器（Vertex Shader）：负责坐标变换、皮肤动画等。
  - 片元着色器（Fragment Shader）：负责逐像素颜色、光照计算。
  这让渲染流程从“固定流程”变成了“可编排流程”，也让 Vulkan、DirectX 12、Metal 这类现代 API 有了更接近硬件的控制能力。

### 实践建议
入门第一步，建议先用 GLFW 加上 GLAD 搭建最小环境，加载着色器程序并绘制一个静态彩色三角形。这个练习虽然简单，却能帮你快速认识几个关键对象：
- VAO（Vertex Array Object）：绑定顶点属性配置
- VBO（Vertex Buffer Object）：存储顶点数据
- Shader Program：链接顶点与片元着色器
当三角形成功渲染出来后，再尝试加入旋转、缩放和颜色动画，你会更直观地理解流水线是怎么工作的。
